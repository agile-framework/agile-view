...
/**
 * pageTitle - 设置页面标签的指令 - mata title
 */
function pageTitle($rootScope, $timeout) {
    return {
        link: function(scope, element) {
            var listener = function(event, toState, toParams, fromState, fromParams) {
                var pageTitle = '线上线下一体化';
                var moduleTitle = '';
                var rootTitle = '主页';
                if (toState.data && toState.data.pageTitle) pageTitle = toState.data.pageTitle;
                if (toState.data && toState.data.moduleTitle) moduleTitle = toState.data.moduleTitle;
                if (toState.data && toState.data.rootTitle) rootTitle = toState.data.rootTitle;
                $timeout(function() {
                    scope.pageTitle = pageTitle;
                    scope.moduleTitle = moduleTitle;
                    scope.rootTitle = rootTitle;
                    element.text('商品中心 | '+pageTitle);
                });
            };
...
/**
 * sideNavigation - 侧边栏导航运行metsiMenu指令
 */
function sideNavigation($timeout) {
    return {
        restrict: 'A',
        link: function(scope, element) {
            // 触发 metsiMenu 插件，呼叫左侧导航菜单
            $timeout(function(){
                element.metisMenu();
            });
            // 初始化固定左侧菜单
...
/**
 * landingScrollspy - 窗口滚动时将顶部导航栏固定偏移量
 */
...
/**
 * agileDatatable - agile表格标签
 */
function agileDatatable() {
    return {
        restrict: "E",
        template:'<table datatable="" dt-options="dtOptions" dt-columns="dtColumns" dt-instance="dtInstance" dt-disable-deep-watchers="true" class="table table-striped table-hover dataTables-example"></table>',
        controllerAs:'datatableCtrl',
        controller:function($compile,$scope, $element, $attrs, DTOptionsBuilder, DTColumnBuilder, $translate,  commonService, global){
            var $this = this;
            $this.selected = {};
            ...
            //行折叠开关
            var responsive = (""== $attrs.responsive) ? true:false;
            //复选框开关
            var checkbox = (""== $attrs.checkbox) ? true:false;
            //顶部按钮组
            if(angular.fromJson($attrs.topbuttons).length>0){
                angular.forEach(angular.fromJson($attrs.topbuttons),function(topbutton,index){
                    if(topbutton.hasOwnProperty('action')){
                        var actionstr = topbutton.action.split('.');
                        topbutton.action = function(){
                            $scope[actionstr[0]][actionstr[1]]($this.selected);
                        }
                    }
                    $this.topbuttons.push(topbutton);
                });
            }
            var dtOptions = DTOptionsBuilder
                ...
                .withOption('ajax', {
                    ...
                    url: global.serviceAddress+global[$attrs.src]['address'],
                    type:'get',
                    dataFilter: function(data){
                        var json = angular.fromJson(data);
                        var returnjson = {};
                        var dataprops = global[$attrs.src]['dataprop'].split('.');
                        if(dataprops.length>0){
                            angular.forEach(dataprops,function(dataprop,index){
                                if(index<dataprops.length-1){
                                    json = json[dataprop];
                                }
                            });
                        }
                        returnjson.recordsTotal = json.recordsTotal;
                        returnjson.recordsFiltered = json.recordsFiltered;
                        returnjson.data = json[dataprops[dataprops.length-1]];
                        returnjson.draw = json.draw;
                        return angular.toJson(returnjson);
                    }
                })
                .withLanguage({
                    sEmptyTable:     $translate.instant("sEmptyTable"),
                    sInfo:           $translate.instant("sInfo"),
                    sInfoEmpty:      $translate.instant("sInfoEmpty"),
                    sInfoFiltered:   $translate.instant("sInfoFiltered"),
                    sInfoPostFix:    $translate.instant("sInfoPostFix"),
                    sInfoThousands:  $translate.instant("sInfoThousands"),
                    sLengthMenu:     $translate.instant("sLengthMenu"),
                    sLoadingRecords: $translate.instant("sLoadingRecords"),
                    sProcessing:     $translate.instant("sProcessing"),
                    sSearch:         $translate.instant("sSearch"),
                    sZeroRecords:    $translate.instant("sZeroRecords"),
                    oPaginate: {
                        sFirst:    $translate.instant("sFirst"),
                        sLast:     $translate.instant("sLast"),
                        sNext:     $translate.instant("sNext"),
                        sPrevious: $translate.instant("sPrevious")
                    },
                    oAria: {
                        sSortAscending:  $translate.instant("sSortAscending"),
                        sSortDescending: $translate.instant("sSortDescending")
                    }
                })
                .withPaginationType('full_numbers')
                .withDOM('<"html5buttons"B>rft<"bottom"ip>')
                ...
                .withButtons($this.topbuttons)
                .withOption('processing', true)
                .withOption('serverSide', true)
                .withOption('responsive', responsive);
            //复选框列
            var titleHtml = '<input type="checkbox" ng-click="datatableCtrl.toggleAll($event,datatableCtrl.selected)">';
            if(checkbox){
                dtColumns.push(DTColumnBuilder.newColumn(null).withTitle(titleHtml).notSortable()
                    .renderWith(function(data, type, full, meta) {
                        $this.selected[full.id] = false;
                        return '<input type="checkbox" ng-model="datatableCtrl.selected[' + data.id + ']" ng-click="datatableCtrl.toggleOne(datatableCtrl.selected)">';
                    }));
            }
            //普通列
            angular.forEach(angular.fromJson($attrs.columns),function (column,index) {
                if(column.class){
                    dtColumns.push(DTColumnBuilder.newColumn(column.value).withTitle(column.title).withClass(column.class));
                    return;
                }
                dtColumns.push(DTColumnBuilder.newColumn(column.value).withTitle(column.title));
            });
            //操作列
            $this.value = {};
            if(angular.fromJson($attrs.buttons).length>0){

                dtColumns.push(DTColumnBuilder.newColumn(null).withTitle('操作').notSortable().withClass('text-center')
                    .renderWith(function(data, type, full, meta) {
                        var buttons = angular.element('<div class="btn-group"></div>');
                        angular.forEach(angular.fromJson($attrs.buttons),function (button,index) {
                            button.class = button.class ? button.class : 'btn-default';
                            button.active = button.active ? button.active : '';
                            if(button.stateColumn && button.stateValue){
                                if(full[button.stateColumn] == button.stateValue)return;
                            }
                            $this.value[data.id] = data;
                            buttons.append(angular.element('<button class="btn btn-xs btn-outline '+button.class+'" ng-click="'+button.active+'(datatableCtrl.value[' + data.id + '])">'+button.title+'</button>'));
                        });
            ...
            $this.reloadData = function() {
                var conditions = angular.element('[for="'+$attrs.id+'"]');
                var conditionStr = '';
                angular.forEach(conditions,function(condition,index){
                    if("undefined" !== typeof $scope[condition.name]){
                        conditionStr += condition.name+'='+$scope[condition.name]+'&';
                    }
                });
                if (conditionStr.length > 0 ) conditionStr = '?'+conditionStr.substring(0, conditionStr.length-1);
                $scope.dtOptions['ajax']['url'] = global.serviceAddress+global[$attrs.src]['address']+conditionStr;
                $scope.dtInstance.rerender();
                angular.element($scope.selectAll).prop('checked',false);
            };

            angular.element('body').keyup(function (e) {
            ...
            function toggleAll (event, selectedItems) {
                for (var id in selectedItems) {
                    if (selectedItems.hasOwnProperty(id)) {
                        $scope.selectAll = event.target;
                        selectedItems[id] = event.target.checked;
                    }
                }
            }
...
function agileZtree(){
    return {
        restrict: "E",
        replace: true,
        template:'<ul class="ztree" id="{{treeId}}"></ul>',
        controllerAs:'ztreeCtrl',
        controller:function($compile,$scope, $element, $attrs, global,commonService){
            $scope.treeId = new Date().getTime();
            var check = (""==$attrs.check)?true:false;
            this.onCheck =function () {
                if($attrs.keyfor && $attrs.valuefor){
                    var zTree = $.fn.zTree.getZTreeObj($scope.treeId),
                        nodes = zTree.getCheckedNodes(true), v = "",k = "";
                    nodes.sort(function compare(a,b){return a.id-b.id;});
                    for (var i=0, l=nodes.length; i<l; i++) {
                        v += nodes[i].name + ",";
                        k += nodes[i].id + ",";
                    }
                    if (v.length > 0 ) v = v.substring(0, v.length-1),k = k.substring(0, k.length-1);
                    $('input[name="'+$attrs.valuefor+'"]').attr("value", v);
                    $scope[$attrs.keyfor]=k;
                }
            };
            ...
            commonService.getServerData(global[$attrs.src]['address'],global[$attrs.src]['dataprop']).then(function (response) {
...
/**
 * agileInputZtree - agile输入框树
 */
function agileInputZtree(){
    return {
        restrict: "E",
        replace: true,
        template:'<input class="form-control" ng-click="showZtree()" name="{{ztreeName}}" readonly>',
        controllerAs:'ztreeInputCtrl',
        controller:function($compile,$scope, $element, $attrs, global,commonService,$rootScope){
            var name = $attrs.inputname + new Date().getTime();
            $scope.ztreeName = name;
            var menuContent = $compile(angular.element('<div class="ztreeContent" style="display:none; position: absolute;z-index: 999999;background-color: #fff;border: 1px solid #e5e6e7"><agile-ztree src="productDir" keyfor="'+$attrs.inputname+'" valuefor="'+name+'" check></agile-ztree></div>'))($scope);
            angular.element('body').append(menuContent);
            $element.after($compile('<input class="form-control" style="display: none;" name="'+$attrs.inputname+'" ng-model="'+$attrs.inputname+'" for="'+$attrs.for+'">')($scope));

            function hideInputZtree($even){
                if (!(angular.element($even.target).attr('name') == $element.attr('name') || angular.element($even.target).parents(".ztreeContent").length>0)) {
                    angular.element('.ztreeContent').slideUp("fast");
                    angular.element('body').unbind('click');
                }
            }
            $scope.showZtree = function () {
                var top = $element.offset().top;
                var left = $element.offset().left;
                angular.element(menuContent).css({left:left + "px", top:top +$element.outerHeight()-1+ "px",width:$element.outerWidth()+ "px"}).slideDown("fast");
                angular.element('body').on('click',hideInputZtree);
            };
            //跳页时清除树面板
            $rootScope.$on('$stateChangeStart',function(){
                angular.element(menuContent).remove();
            })
        }
    }
}
/**
 * agileEchartsLineStack - agile折线堆叠
 */
function agileEchartsLineStack() {
    return {
        scope: {
            legend: "=",
            item: "=",
...
/**
 * agileEchartsMixTimelineFinance - agile混合指标
 */
function agileEchartsMixTimelineFinance() {
    return {
        scope: {
            legend: "=",
            item: "=",
...
angular
    ...
    .directive('agileDatatable', agileDatatable)
    .directive('agileZtree', agileZtree)
    .directive('agileInputZtree', agileInputZtree)
    .directive('agileEchartsLineStack', agileEchartsLineStack)
    .directive('agileEchartsMixTimelineFinance', agileEchartsMixTimelineFinance)
;

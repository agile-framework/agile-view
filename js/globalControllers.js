 ...
 *
*/
function MainCtrl($scope,$state,commonService,global) {
    $this = this;
    $scope.legend = ["商品中心", "订单中心",'计算中心','稽核中心'];
     ...
     * 登陆
     */
    $scope.menusInfo = angular.fromJson(sessionStorage.getItem("menusInfo"));
    $scope.userInfo = angular.fromJson(sessionStorage.getItem("userInfo"));
    $scope.roleInfo = angular.fromJson(sessionStorage.getItem("roleInfo"));
    this.login = function () {
        commonService.getServerData(global['loginInfo']['address'],global['loginInfo']['dataprop'],{name:$this.name,password:$this.password}).then(function(response){
            $scope.menusInfo = response[global.loginInfo.menu];
            $scope.userInfo = response[global.loginInfo.detail];
            $scope.roleInfo = response[global.loginInfo.role];
            sessionStorage.setItem("menusInfo",angular.toJson($scope.menusInfo));
            sessionStorage.setItem("userInfo",angular.toJson($scope.userInfo));
            sessionStorage.setItem("roleInfo",angular.toJson($scope.roleInfo));
    ...
    $scope.nowTime=systemTime.getHours()+':'+systemTime.getMinutes()+':'+systemTime.getSeconds();
    ...
    this.SetTimerInterval=setInterval(this.setTimer,1000);
    // /**
    //  * 获取菜单数据
    //  */
    // commonService.getServerData(global.menu.address,global.menu.dataprop).then(function (response) {
    //     $scope.menusInfo = response;
    // });
    /**
     * 表单提交值
     */
    $scope.formValue = {};
    this.pageJump = function (pageUrl) {
        $state.go(pageUrl,$scope.formValue);
        $scope.formValue = {};
        console.info($scope.formValue);
    }
    /**
     * 国家 - Used as duallistbox in form advanced view
     */
    this.countries = [
        { name: 'Amsterdam' },
        { name: 'Washington' },
    ...
    this.getLocation = function(val) {
        return commonService.getLocation(val).then(function(response){
    ...
    /**
     * 日期范围 - 用于表单的日期控件初始化时间范围
     */
    this.daterange = {startDate: null, endDate: null};

    /**
     * 滑动间隔 - 用于bootstrap的滑动效果 , 单位毫秒:
     */
    ...
    /**
     * check's - 对于复选框输入用于检查插件的几个变量。仅用于演示目的
     */
    this.checkOne = true;
    this.checkTwo = true;
     ...
     */
    this.alerts = [
        { type: 'danger', msg: '操作有误，请重新操作' },
        { type: 'success', msg: '操作成功' },
        { type: 'info', msg: '这是一般提示信息' }
    ];

    /**
     * addAler添加提示, closeAlert关闭提示  - 用于通知和提示页面管理通知
     */
...
function configCtrl($scope,$cookies) {

    this.fixednavbar = function (onOff) {
        if ($('#fixednavbar').is(':checked') && onOff) {
            $(".navbar-static-top").removeClass('navbar-static-top').addClass('navbar-fixed-top');
            $("body").removeClass('boxed-layout');
            $("body").addClass('fixed-nav');
            $('#boxedlayout').prop('checked', false);
            $cookies.put('fixednavbar',1);
            $cookies.put('boxedlayout',0);
        } else {
            $(".navbar-fixed-top").removeClass('navbar-fixed-top').addClass('navbar-static-top');
            $("body").removeClass('fixed-nav');
            $cookies.put('fixednavbar',0);
        }

    };
    this.fixedsidebar = function (onOff) {
        if ($('#fixedsidebar').is(':checked') && onOff) {
            $("body").addClass('fixed-sidebar');
            $('.sidebar-collapse').slimScroll({
                height: '100%',
                railOpacity: 0.9
            });
            $cookies.put('fixedsidebar',1);
        } else {
            $('.sidebar-collapse').slimscroll({destroy: true});
            $('.sidebar-collapse').attr('style', '');
            $("body").removeClass('fixed-sidebar');
            $cookies.put('fixedsidebar',0);
        }
    };
    this.collapsemenu = function (onOff) {
        if ($('#collapsemenu').is(':checked') && onOff) {
            $("body").addClass('mini-navbar');
            this.smoothlyMenu();
            $cookies.put('collapsemenu',1);
        } else {
            $("body").removeClass('mini-navbar');
            this.smoothlyMenu();
            $cookies.put('collapsemenu',0);
        }
    };
    this.boxedlayout = function (onOff) {
        if ($('#boxedlayout').is(':checked') && onOff) {
            ...
            $cookies.put('boxedlayout',1);
            $cookies.put('fixednavbar',0);
            $cookies.put('fixedfooter',0);
        } else {
            $("body").removeClass('boxed-layout');
            $cookies.put('boxedlayout',0);
        }
    };
    this.fixedfooter = function (onOff) {
        if ($('#fixedfooter').is(':checked') && onOff) {
            $('#boxedlayout').prop('checked', false);
            $("body").removeClass('boxed-layout');
            $(".footer").addClass('fixed');
            $cookies.put('fixedfooter',1);
            $cookies.put('boxedlayout',0);
        } else {
            $(".footer").removeClass('fixed');
            $cookies.put('fixedfooter',0);
    ...
    this.spinIcon = function () {
        $(".theme-config-box").toggleClass("show");
    };
    this.skin = function (index) {
        switch(index)
        {
            case 1:
    ...
    this.reset = function(){
        var fixednavbar = ($cookies.get('fixednavbar') && $cookies.get('fixednavbar')==1)?true:false;$('#fixednavbar').prop('checked',fixednavbar);this.fixednavbar(fixednavbar);
        var fixedsidebar = ($cookies.get('fixedsidebar') && $cookies.get('fixedsidebar')==1)?true:false;$('#fixedsidebar').prop('checked',fixedsidebar);this.fixedsidebar(fixedsidebar);
        var collapsemenu = ($cookies.get('collapsemenu') && $cookies.get('collapsemenu')==1)?true:false;$('#collapsemenu').prop('checked',collapsemenu);this.collapsemenu(collapsemenu);
        var boxedlayout = ($cookies.get('boxedlayout') && $cookies.get('boxedlayout')==1)?true:false;$('#boxedlayout').prop('checked',boxedlayout);this.boxedlayout(boxedlayout);
        var fixedfooter = ($cookies.get('fixedfooter') && $cookies.get('fixedfooter')==1)?true:false;$('#fixedfooter').prop('checked',fixedfooter);this.fixedfooter(fixedfooter);
        var skin = $cookies.get('skin')?Number($cookies.get('skin')):0;this.skin(skin);
    };
    this.reset();
}
...
// 依赖注入
MainCtrl.$inject = ['$scope','$state','commonService','global'];
configCtrl.$inject = ['$scope','$cookies'];

// console.info(angular.injector().annotate(configCtrl));
